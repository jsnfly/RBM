...
class RBM:
    ...
    def train_rbm(self, train_data, epochs=10, batch_size=32, summary_path=None, summary_frequency=10,
        ...
        if isinstance(train_data, np.ndarray):
            ...
            train_dataset = make_ds.simple_dataset(train_data, batch_size,
                                                   shuffle_buffer=shuffle_buffer_size, cache=True)
    ...
    def train_rbm_enhanced(self, train_data, epochs=10, batch_size=32, summary_path=None, summary_frequency=10,
                           dbn=None, update_vbiases=True, start_learning_rate=0.01, learning_rate_decay=(10, 1.0),
                           cd_steps=1, sparsity_rate=0.0, sparsity_goal=0.1,
                           adaptive_learning_rate=False, enhanced_gradient=False):
        ...
        if adaptive_learning_rate:
            # get one batch
            first_batch = train_utils.get_one_batch(train_data, batch_size)

        # make datasets, if necessary also for softmax training
        train_dataset = make_ds.simple_dataset(train_data, batch_size, shuffle_buffer=shuffle_buffer_size, cache=True)

        # initializable iterator
        iterator = tf.data.Iterator.from_structure(train_dataset.output_types, train_dataset.output_shapes)

        # get batches from datasets
        # for adaptive learning rate two batches are needed
        if adaptive_learning_rate:
            batch = tf.Variable(first_batch, validate_shape=False, dtype=tf.float32)
        ...
        v0_ = train_utils.upward_propagation(batch, dbn, self.layer_index)
        if adaptive_learning_rate:
            next_v0_ = train_utils.upward_propagation(next_batch, dbn, self.layer_index)
        ...
        if adaptive_learning_rate:
            ...
            etas = tf.stack([tf.multiply((1 - epsilon), learning_rate),
                             learning_rate,
                             tf.multiply((1 + epsilon), learning_rate)], axis=0)
        ...
        with tf.name_scope('calculate_deltas'):
            if enhanced_gradient:
                delta_vbiases, delta_hbiases, delta_weights = deltas.enhanced_deltas(v0_, h0_, vn_, hn_, batch_size)
                if self.layer_type == 'gb' or self.layer_type == 'gr':
                    delta_log_sigmas = deltas.enhanced_delta_log_sigmas(v0_, h0_, vn_, hn_,
                                                                        train_vbiases,
                                                                        train_weights,
                                                                        train_log_sigmas)
            else:
        ...
        if adaptive_learning_rate:
            if self.layer_type == 'gb' or self.layer_type == 'gr':
                ind_eta = train_utils.get_best_learning_rate(next_v0_, self.layer_type, etas, cd_steps,
                                                             train_vbiases, train_hbiases, train_weights,
                                                             delta_vbiases, delta_hbiases, delta_weights,
                                                             train_log_sigmas, delta_log_sigmas)
            else:
                ind_eta = train_utils.get_best_learning_rate(next_v0_, self.layer_type, etas, cd_steps,
                                                             train_vbiases, train_hbiases, train_weights,
                                                             delta_vbiases, delta_hbiases, delta_weights)

            assign_eta = tf.assign(learning_rate, tf.maximum(tf.minimum(etas[ind_eta], max_eta), min_eta))

        with tf.name_scope('updates'):
            if adaptive_learning_rate:
                with tf.control_dependencies([assign_eta]):
                    assign_batch = tf.assign(batch, next_batch, validate_shape=False)
                ...
                # assign operations (are executed when session is run)
                assign_ops = [assign_eta, assign_batch, assign_err, assign_q, assign_avg_activation,
                              assign_vbiases, assign_hbiases, assign_weights]
                if self.layer_type == 'gb' or self.layer_type == 'gr':
                    assign_ops.append(assign_log_sigmas)
