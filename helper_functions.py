...
def plot_PCA_and_TSNE(data,labels,save_path,save_name,one_hot_labels=True,
    ...
    for i in range(num_classes):
        ...
        ax.scatter(pca_samples_class[:,0],pca_samples_class[:,1],color=cmap[i],label='class {}'.format(i),alpha=0.8)
    ax.legend()
    ...
    for i in range(num_classes):
        ...
        ax.scatter(tsne_samples_class[:,0],tsne_samples_class[:,1],color=cmap[i],label='class {}'.format(i),alpha=0.8)
    ax.legend()
...
def layerwise_activations(DBN,data,num_activations,normalize=False):
    ...
    one_epoch_activations = [] # list of lists where each inner list has one activation-batch for each layer
    batch_size = 256
    dataset = make_ds.simple_dataset(data,batch_size,shuffle=False,cache=True)
...
def extract_value(DBN_train_params,key,li=0,default_value=None):
    ...
    Args:
        DBN_params: dictionary of training parameters, dictionary
